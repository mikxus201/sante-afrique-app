...
export default function AbonnementPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await getMySubscription(); // attendu: { data: Sub }
  ...
  const started = sub.started_at ? new Date(sub.started_at).toLocaleDateString("fr-FR") : "—";
  const ends = sub.ends_at ? new Date(sub.ends_at).toLocaleDateString("fr-FR") : "—";
    ...
    <div className="space-y-6">
      <section className="rounded border bg-white p-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {isExpired && (
            <span className="text-sm text-red-600">Votre abonnement est arrivé à expiration.</span>
          )}
        </div>
      </section>
