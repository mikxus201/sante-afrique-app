  ...
  if (!sub || sub.status === "none")
    return (
      <section className="rounded border bg-white p-6">
        ...
        <p className="mt-2 text-neutral-700">Abonnez-vous pour accéder à 100% de nos contenus.</p>
        <div className="mt-4">
    ...
    <div className="space-y-6">
      <section className="rounded border bg-white p-6">
        ...
        <p className="mt-2 text-neutral-700">
          {sub.plan?.name ? <>Formule : <strong>{sub.plan.name}</strong>. </> : null}
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          {sub.status === "expired" && <span className="text-sm text-red-600">Votre abonnement est expiré.</span>}
        </div>
      </section>
      ...
      <section className="rounded border bg-white p-6">
        ...
        <ul className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {["Tous les articles en illimité", "Le magazine en avant-première", "Exclusivités abonnés", "Conseils pratiques", "Accès premium à l’app"].map((t, i) => (
            <li key={i} className="rounded border bg-neutral-50 px-4 py-3">{t}</li>
          ))}
        </ul>
      </section>
    </div>
