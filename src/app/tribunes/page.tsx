...
async function fetchArticles(params: {
  ...
  const all: Article[] = Array.isArray(payload)
    ? payload
    : payload?.items ?? payload?.data ?? [];
  const needle = "tribune"; // match en minuscules
  ...
  const paginator: Paginator<Article> = {
    ...
    next_page_url:
      current_page < last_page
        ? `/tribunes?page=${current_page + 1}&sort=${params.sort}${
...
export const metadata = {
  title: `Tribunes — Santé Afrique`,
  description: "Opinions et analyses.",
};

export default async function TribunesPage({
  searchParams,
}: {
  searchParams?: { page?: string; sort?: string; q?: string };
}) {
  const page = Math.max(1, Number(searchParams?.page || "1"));
  const sort = (searchParams?.sort === "views" ? "views" : "date") as "date" | "views";
  const q = (searchParams?.q || "").trim() || undefined;

  const paginator = await fetchArticles({ page, sort, q, perPage: 12 });

  const buildHref = (over: Partial<{ page: number; sort: "date" | "views"; q?: string }>) => {
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      {paginator.data.length === 0 ? (
        <div className="text-sm text-neutral-500">
          Aucune tribune trouvée (vérifie le nom exact “{CATEGORY}”).
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {paginator.data.map((a) => {
            const img = articleImg(a);
            const catText = pickCategory(a);
            return (
      ...
      {paginator.last_page > 1 && (
        <div className="mt-8 flex items-center justify-between">
          <div className="text-sm text-neutral-500">
            Page {paginator.current_page} / {paginator.last_page} — {paginator.total} tribunes
          </div>
