...
function renderOption(item: unknown, idx: number) {
  if (typeof item === "string") {
    const v = item.trim();
    return <option key={(v || idx).toString()} value={v}>{v}</option>;
  }
  const o = item as any;
  const value: string = String(o?.code ?? o?.value ?? o?.id ?? o?.name ?? "");
  const label: string = String(o?.name ?? o?.label ?? value);
  return <option key={(value || `opt-${idx}`).toString()} value={value}>{label}</option>;
}

function filenameFromUrl(u?: string | null): string {
  if (!u) return "cv.pdf";
  try { return decodeURIComponent((u.split("?")[0].split("/").pop() || "cv.pdf")); }
  catch { return "cv.pdf"; }
}
...
export default function CandidatesListPage() {
  ...
  const professionsList = useMemo(() => {
    ...
    const inferred = Array.from(set).sort((a, b) => a.localeCompare(b, "fr", { sensitivity: "base" }));
    ...
    (async () => {
      ...
      try {
        const res = await fetchRecruiterCandidates({
          q, country, profession, type, availability,
          experience_min: experienceMin || undefined,
          page: 1, per_page: 50,
        });

        if (res?.meta?.subscription_required) {
  ...
  function exportCsv() {
    ...
    const rows = [
      ["Nom","Email","Téléphone","Pays","Ville","Profession","Expérience (ans)","Type souhaité","Disponibilité"],
      ...items.map((c) => [
    ...
    a.href = url; a.download = "candidats_sante_afrique.csv"; document.body.appendChild(a); a.click(); a.remove();
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={exportCsv} className="inline-flex rounded bg-neutral-900 px-3 py-1.5 text-sm font-semibold text-white hover:opacity-90" disabled={!items.length}>
            Exporter CSV
          </button>
        </div>
      </div>
      ...
      <form onSubmit={submitFilters} className="mt-6 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
        <div className="md:col-span-2">
          <label className="block text-xs font-semibold mb-1">Recherche</label>
          <input name="q" defaultValue={q} placeholder="Nom, email, mots clés…" className="w-full border rounded px-3 py-2" />
        </div>
        ...
        <div>
          ...
          <select name="country" defaultValue={country} className="border rounded px-3 py-2">
            ...
            {(Array.isArray(FRANCOPHONE_COUNTRIES) ? FRANCOPHONE_COUNTRIES : []).map((c, i) => renderOption(c, i))}
          </select>
        </div>
        ...
        <div>
          ...
          <input name="experience_min" defaultValue={experienceMin} type="number" min={0} className="w-full border rounded px-3 py-2" />
        </div>
      ...
      {loading && <section className="rounded border bg-white p-6 mt-6">Chargement…</section>}
      ...
      {!loading && (err || subscriptionRequired) && (
        <section className="rounded border bg-white p-6 mt-6">
          ...
          <div className="mt-4">
            <Link href="/abonnement" className="inline-flex rounded bg-neutral-900 px-4 py-2 text-white hover:opacity-90">
                ...
                return (
                  <tr key={String(c.id)} className="border-t">
                    ...
                    <td className="px-4 py-2">
                      ...
                      <div className="text-xs text-neutral-500">
                        Exp. {c.experience_years ?? 0} an(s) • {c.desired_type || "Type ?"} • {c.availability || "Dispo ?"}
                      </div>
                    </td>
                    <td className="px-4 py-2">
                      {c.cv_url ? (
                        <a href={c.cv_url} target="_blank" className="inline-flex items-center gap-2 rounded bg-blue-600 px-3 py-1.5 text-white font-semibold hover:bg-blue-700">
